...
jq -nc --arg s "$root/hooks/mem-save.sh" '{decision:"block", reason:("Files were edited this session. In your FINAL message (Conductor shows only your last message, so make it self-contained): give a 1-2 line recap of what you did, THEN ask the user (brief) whether to save this change to carryover memory. If yes, EXTRACT the knowledge as JSON and save it via Bash: bash \"" + $s + "\" --json '\''<json>'\'' — where json = {\"content\":\"1-line summary\",\"facts\":[\"atomic, self-contained facts\"],\"entities\":[{\"entity\":\"X\",\"type\":\"project|tech|person|concept\"}],\"relationships\":[{\"source\":\"X\",\"relationship\":\"uses\",\"destination\":\"Y\"}],\"category\":\"area\",\"tags\":[\"...\"],\"importance\":0.7}. Each fact becomes a searchable memory; include entities and relationships for the graph. If structuring is not worth it: bash \"" + $s + "\" \"text\". Ask only once; if the user already answered, just finish.")}'
